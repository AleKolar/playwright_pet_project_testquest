## Тесты для «Калькулятора смет» (`Калькулятор_смет/tests/test_calculator.py`)
...
## Тесты подтверждения дефектов (`test_defects.py`)

### Назначение

Файл `test_defects.py` — это отдельный набор тестов, который **фиксирует известные дефекты выгрузки Excel** из калькулятора смет. Тесты описывают **корректное поведение** системы и помечены маркером `@pytest.mark.xfail` с указанием номера бага в `reason`.
